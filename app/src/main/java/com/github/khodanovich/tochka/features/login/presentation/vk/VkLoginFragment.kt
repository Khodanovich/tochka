...
class VkLoginFragment : GlobalFragment(), VKCallback<VKAccessToken> {
    ...
    private fun startSignInActivity() =
        startActivityForResult(makeIntent(), VKServiceActivity.VKServiceType.Authorization.outerCode)

    private fun makeIntent() = Intent(context, VKServiceActivity::class.java).apply {
        putExtra(KEY_TYPE, VKServiceActivity.VKServiceType.Authorization.name)
